...
import { AiOutlineWechat,AiTwotoneBell,AiFillCaretDown } from "react-icons/ai";
...
function Navbar() {
  const { activeMenu,setIsActiveMenu, isClicked, 
    setIsclicked, handleClicked, screen, setScreen } = useStateContext();
    ...
    return (
      <TooltipComponent content={title}>
        <button 
         ...
         style={{color}}
        >
            <span style={{backgroundColor:dotColor}} className="absolute w-2 h-2 -t-2 rounded-full"></span>
            {icon}
        </button>
      </TooltipComponent>
    );
        ...
        <div className="flex">
            ...
            title="Notification"
             ...
             <div className="flex items-center  mr-3">
                    <img className="w-8 h-8 mx-2 rounded-full " src={avatar} alt="avatar" />
                    <span >Hi, </span> {' '}
                        <span className="font-bold">Hung Le</span>
                        <button onClick={()=>{
